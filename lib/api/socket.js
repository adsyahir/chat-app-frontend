...
const setupSocketListeners = (userId) => {
  ...
  // The server now authenticates the handshake from the session cookie, so an
  // expired session surfaces here instead of silently connecting as nobody.
...
export const socketAPI = {
  connect: () => {
    ...
    // No userId in the query: identity comes from the session cookie the
    // handshake carries, so it cannot be spoofed by the client.
