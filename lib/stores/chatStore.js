  ...
  return create(
    persist(
      (set, get) => ({
        ...
        onlineUsers: [],
        socket: null,
        // senderId -> count of messages received while their chat was closed.
        unreadBySender: {},
        setSelectedContact: (contact) =>
          set((state) => ({
            selectedContact: contact,
            // Opening a chat clears its badge.
            unreadBySender: Object.fromEntries(
              Object.entries(state.unreadBySender).filter(
                ([id]) => id !== (contact?.friendId ?? contact?._id)
              )
            ),
          })),
        incrementUnread: (senderId) =>
          set((state) => ({
            unreadBySender: {
              ...
              [senderId]: (state.unreadBySender[senderId] ?? 0) + 1,
            },
          })),
        clearUnread: (senderId) =>
          set((state) => {
            const next = { ...state.unreadBySender };
        ...
        reset: () => set({ selectedContact: null, socket: null, unreadBySender: {} }),
      }),
      {
